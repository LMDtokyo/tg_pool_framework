"""
src/accounts/warmup_policy.py — Ramp new accounts up gradually instead of using
...
Account age comes from RegistryEntry.first_seen (src/accounts/account_registry.py),
which is backed by AccountRow.created_at (src/db/models.py) when persistence is
