...
internal static class DatamollStrings
{
    ...
    {
        table.Add("Datamoll.SectionProvider", "ПЛАТЕЖНЫЙ СЕРВИС", "PAYMENT SERVICE", "支付服务");
        table.Add("Datamoll.ServiceLabel", "Сервис", "Service", "服务");
        table.Add("Datamoll.ApiKeyLabel", "Платежный API-ключ", "Payment API key", "支付 API 密钥");
        table.Add("Datamoll.ApiKeyTooltip", "API-ключ, выданный платежным сервисом", "API key issued by the payment service", "支付服务签发的 API 密钥");
        table.Add("Datamoll.CredentialsNote",
            "Ключ хранится только в памяти и используется для баланса и покупок.",
            "The key stays in memory and is used for balance and purchases.",
            "密钥仅保存在内存中，用于余额查询和购买。");
        ...
        table.Add("Datamoll.ActivitySubtitle", "Актуальный статус заказов Andromeda service", "Live Andromeda service order status", "Andromeda service 实时订单状态");
        ...
        table.Add("Datamoll.SectionDelivery", "ДОСТАВКА ANDROMEDA SERVICE", "ANDROMEDA SERVICE DELIVERY", "ANDROMEDA SERVICE 交付");
        ...
        table.Add("Datamoll.DeliveryNote1",
            "Покупки реальны и списывают отображаемый платежный баланс. Наличие и цена проверяются повторно при оформлении заказа.",
            "Purchases are real and deduct the displayed payment balance. Stock and price are checked again when the order is placed.",
            "购买是真实的，将从显示的支付余额中扣除。下单时将再次检查库存和价格。");
        table.Add("Datamoll.DeliveryNote2",
            "Повторная попытка использует тот же идентификатор, позволяя Andromeda service восстановить задержанный заказ без двойного списания.",
            "A retry reuses the same order identity, allowing Andromeda service to recover delayed orders without charging twice.",
            "重试将复用相同的订单标识，使 Andromeda service 能够恢复延迟订单而不重复扣费。");
