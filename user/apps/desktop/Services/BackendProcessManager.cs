...
/// <summary>
/// Launches the Python local-agent API (tg_pool/api/app.py, via uvicorn) as a child
...
public sealed class BackendProcessManager : IAsyncDisposable
{
    ...
    /// (via the LOCAL_API_TOKEN env var, see tg_pool/api/local_auth.py) -- lets
    ...
    public async Task<bool> StartAndWaitReadyAsync(TimeSpan timeout, CancellationToken ct = default)
    {
        ...
        var psi = new ProcessStartInfo
        {
            ...
            Arguments = $"-m uvicorn tg_pool.api.app:app --host 127.0.0.1 --port {_port} --no-access-log",
