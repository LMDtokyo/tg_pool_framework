...
/// gate (tg_pool/api/license_gate.py) picks it up too.
/// </summary>
public static class LicenseGateway
{
    ...
    public static async Task<bool> RunActivatorAsync(string localAgentRoot, string dataDir)
    {
        var exePath = ResolveActivatorExecutable(localAgentRoot);
        if (exePath is null)
            return false;
    ...
    private static string? ResolveActivatorExecutable(string localAgentRoot)
    {
        string[] candidates =
        [
            Path.Combine(AppContext.BaseDirectory, "TgPoolActivator.exe"),
            Path.GetFullPath(Path.Combine(localAgentRoot, "..", "..", "apps", "activator", "bin", "Release", "net10.0-windows", "TgPoolActivator.exe")),
            Path.GetFullPath(Path.Combine(localAgentRoot, "..", "..", "apps", "activator", "bin", "Debug", "net10.0-windows", "TgPoolActivator.exe")),
        ];
