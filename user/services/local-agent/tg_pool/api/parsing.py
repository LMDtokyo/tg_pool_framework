"""
tg_pool/api/parsing.py — Single-parsing-job task lifecycle + headless progress tracking.
...
finished. Uses PoolAccessGuard (tg_pool/api/pool_guard.py) to reserve the
...
orchestrate_extraction_only() (tg_pool/orchestrator.py) publishes both when
given an event_bus.

Unlike the CLI's tg_pool/features/parsing.py (which reads PARSE_* env vars),
...
from tg_pool.extraction.data_extraction import AutoStrategySelector, BaseParsingStrategy
from tg_pool.extraction.exporter import DataExporter
from tg_pool.extraction.user_filter import UserFilterPipeline
from tg_pool.monitoring.event_bus import AccountStatusEvent, EventBus, MetricUpdateEvent
