"""
tg_pool/account_loader.py — Multi-format account loader with Round-Robin proxy allocation.
...
async def load_from_tdata(
    ...
    Delegates to TDataConverter (tg_pool/proxy/tdata_converter.py) so tdata loading
...
def _write_fingerprint_sidecar(
    ...
    tg_pool/api/json_generator.py writes), so every future load finds the .json
