"""
tg_pool/bootstrap.py — Shared env-driven bootstrap for every feature entry point.
...
of duplicated per feature module (tg_pool/features/*.py) or left trapped in
...
def load_accounts(load_failures: Optional[List[dict]] = None) -> Tuple[List[AccountConfig], List[AccountConfig]]:
    from tg_pool.accounts.account_loader import load_accounts_from_directory
...
async def load_tdata_accounts(load_failures: Optional[List[dict]] = None) -> List[AccountConfig]:
    ...
    via load_from_tdata() (tg_pool/accounts/account_loader.py -> TDataConverter).
    ...
    import json
    from tg_pool.accounts.account_loader import load_from_tdata
    from tg_pool.proxy.tdata_converter import find_tdata_folders
...
def build_db_session_factory():
    ...
    if not database_url:
        return None
    from tg_pool.db.engine import build_session_factory
...
def build_health_checker(policy, event_bus):
    from tg_pool.accounts.health_checker import HealthChecker
