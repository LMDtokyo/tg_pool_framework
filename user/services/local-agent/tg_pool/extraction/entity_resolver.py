"""
tg_pool/entity_resolver.py — Universal Entity Resolver.
...
class EntityInfo:
    ...
    in tg_pool.extraction.data_extraction). Falls back to a safe default
    ...
    principle as tg_pool.accounts.geo.country_for_phone().
    """
    weight: float = 1.0
...
async def estimate_entity_weight(client: TelegramClient, identifier: str) -> float:
    ...
    (see tg_pool.orchestrator._distribute_by_weight). Never raises.
    """
