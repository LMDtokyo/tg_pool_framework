"""
tg_pool/db/models.py — SQLAlchemy 2.0 declarative models for durable account state.
...
class AccountRow(Base):
    ...
    Mirrors tg_pool.config.AccountConfig + tg_pool.accounts.health_checker.AccountState so that
...
class LicenseStateRow(Base):
    ...
    # server (see tg_pool/licensing/signature.py). Empty for rows written before this
