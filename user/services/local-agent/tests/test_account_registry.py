"""
tests/test_account_registry.py — Tests for tg_pool/account_registry.py,
...
from tg_pool.accounts.account_registry import AccountQuery, AccountRegistry, RegistryEntry
from tg_pool.config import AccountConfig, ProxyConfig
from tg_pool.accounts.health_checker import (
    AccountState,
    AccountStatus,
    HealthChecker,
    HealthResult,
...
from tg_pool.proxy.proxy_checker import (
    ProxyState,
    ProxyType,
...
class TestSpamBotIntegration:
    ...
    async def test_spambot_with_date_sets_restriction_expires(self):
        ...
        with (
            patch("tg_pool.accounts.health_checker.asyncio.sleep", new_callable=AsyncMock),
            patch("tg_pool.accounts.health_checker._SPAMBOT_REPLY_TIMEOUT", 30.0),
        ):
...
class TestCheckProxy:
    async def test_successful_socks5_returns_active(self):
        ...
        with patch("tg_pool.proxy.proxy_checker.asyncio.open_connection", return_value=(reader, writer)):
    ...
    async def test_timeout_returns_inactive_with_message(self):
        with patch(
            "tg_pool.proxy.proxy_checker.asyncio.open_connection",
...
class TestSocks4Connector:
    ...
    async def test_socks4_success(self):
        ...
        with (
            patch("tg_pool.proxy.proxy_checker.asyncio.get_running_loop", return_value=self._make_mock_loop()),
            patch("tg_pool.proxy.proxy_checker.asyncio.open_connection", return_value=(reader, writer)),
        ):
            state = await check_proxy(
    ...
    async def test_socks4_dns_resolve_failure(self):
        ...
        with patch("tg_pool.proxy.proxy_checker.asyncio.get_running_loop", return_value=loop):
            state = await check_proxy(
                {"type": "socks4", "host": "proxy.example.com", "port": 1080}, retries=0,
            )
...
class TestHttpsConnector:
    async def test_https_proxy_success(self):
        ...
        with patch(
            "tg_pool.proxy.proxy_checker.asyncio.open_connection", return_value=(reader, writer)
        ) as mock_open:
            state = await check_proxy(
...
class TestCheckProxyRetries:
    async def test_retries_until_success(self):
        ...
        with patch(
            "tg_pool.proxy.proxy_checker.asyncio.open_connection", side_effect=flaky_open_connection
        ):
            state = await check_proxy(
    ...
    async def test_zero_retries_means_single_attempt(self):
        ...
        with patch("tg_pool.proxy.proxy_checker.asyncio.open_connection", side_effect=always_fail):
            await check_proxy(
    ...
    async def test_config_error_is_not_retried(self):
        ...
        with patch("tg_pool.proxy.proxy_checker.asyncio.open_connection", side_effect=track_calls):
            state = await check_proxy(
...
class TestCheckAllProxies:
    async def test_returns_one_state_per_proxy_in_order(self):
        ...
        with patch("tg_pool.proxy.proxy_checker.check_proxy", side_effect=fake_check):
