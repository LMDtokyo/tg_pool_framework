"""
tests/test_orchestrator.py — Integration-style tests for tg_pool/orchestrator.py.
...
from tg_pool.orchestrator import _collect_bucket, _distribute_by_weight, _weigh_entities, orchestrate
...
class TestOrchestrateBasic:
    ...
    async def test_full_pipeline_success(self):
        ...
        with (
            ...
            patch(
                "tg_pool.orchestrator.extract_members",
...
class TestOrchestrateDeduplication:
    async def test_usernames_from_multiple_workers_are_merged(self):
        ...
        with (
            patch("tg_pool.orchestrator.ClientPool", return_value=mock_pool),
            patch("tg_pool.orchestrator.extract_members", side_effect=side_effect_extract),
...
class TestWeighEntities:
    async def test_pairs_each_identifier_with_its_estimated_weight(self):
        ...
        with patch("tg_pool.orchestrator.estimate_entity_weight", mock_estimate):
...
class TestCollectBucketAntiflood:
    async def test_no_redis_client_means_no_throttling(self):
        ...
        with patch("tg_pool.orchestrator.extract_users", new=AsyncMock(return_value=users)):
            results = [
    ...
    async def test_different_entities_have_independent_buckets(self):
        ...
        with patch("tg_pool.orchestrator.extract_users", new=AsyncMock(side_effect=fake_extract_users)):
...
class TestCollectBucketStrategySelector:
    ...
    async def test_selector_picks_strategy_per_entity(self):
        ...
        with (
            patch("tg_pool.orchestrator.describe_entity", new=AsyncMock(side_effect=[info_a, info_b])),
