...
from tg_pool.api.send_by_id import (
    IdRecipient,
    SendByIdManager,
    SendByIdOptions,
    load_id_database,
...
def test_delay_multiplier_is_one_without_a_warmup_policy():
    manager = SendByIdManager([make_account("+100")], PoolAccessGuard())
...
async def test_daily_cap_allows_everything_without_warmup_or_redis():
    ...
    manager_no_redis = SendByIdManager(
        [make_account("+100")], PoolAccessGuard(), warmup_policy=WarmupPolicy()
    )
    assert await manager_no_redis._daily_cap_allows("+100") is True
...
def test_assign_recipients_respects_total_sender_capacity(monkeypatch) -> None:
    monkeypatch.setattr("tg_pool.api.send_by_id.random.randint", lambda _a, _b: 2)
...
async def test_manager_runs_job_exports_results_and_releases_pool(
    ...
    monkeypatch.setattr("tg_pool.api.send_by_id.ClientFactory.build", lambda _account: client)
    pool_guard = PoolAccessGuard()
