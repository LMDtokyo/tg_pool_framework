...
from tg_pool.orchestrator import orchestrate_until_target
...
async def test_stops_after_single_cycle_when_target_reached():
    with patch(
        "tg_pool.orchestrator.orchestrate_multi_source",
...
async def test_cycles_accumulate_excludes_and_totals():
    ...
    with patch("tg_pool.orchestrator.orchestrate_multi_source", side_effect=fake_orchestrate):
        report = await orchestrate_until_target(exact_total_target=5, accounts=[])
...
async def test_shutdown_event_already_set_skips_all_cycles():
    ...
    with patch("tg_pool.orchestrator.orchestrate_multi_source") as mock_send:
