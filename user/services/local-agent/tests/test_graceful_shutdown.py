...
from tg_pool.monitoring.event_bus import EventBus, MessageDeliveredEvent
...
class TestOrchestrateShutdown:
    async def test_close_all_called_after_shutdown(self):
        ...
        with (
            ...
            patch("tg_pool.orchestrator.extract_members", new=AsyncMock(return_value=set())),
        ):
            report = await asyncio.wait_for(
                orchestrate(
                    accounts=accounts,
    ...
    async def test_orchestrate_returns_partial_report_on_shutdown(self):
        ...
        with (
            patch("tg_pool.orchestrator.ClientPool", return_value=mock_pool),
            patch("tg_pool.orchestrator.extract_members", side_effect=extract_with_shutdown),
            patch("tg_pool.orchestrator.send_notifications",
...
class TestExtractionCancellation:
    async def test_extraction_cancelled_when_shutdown_fires(self):
        ...
        with (
            patch("tg_pool.orchestrator.ClientPool", return_value=mock_pool),
            patch("tg_pool.orchestrator.extract_members", side_effect=slow_extraction),
        ):
