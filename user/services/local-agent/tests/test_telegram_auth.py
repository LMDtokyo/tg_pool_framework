...
from tg_pool.api.hero_sms_activation import HeroSmsActivationManager
from tg_pool.api.telegram_auth import FingerprintCatalog, TelegramAuthenticator
...
async def test_authenticator_saves_reusable_session_and_metadata(
    ...
    monkeypatch.setattr(
        "tg_pool.api.telegram_auth.ClientFactory.build",
...
async def test_authenticator_uses_registration_proxy_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("tg_pool.api.telegram_auth.ClientFactory.build", build_client)
    authenticator = TelegramAuthenticator(
        fingerprint_file=str(source),
        accounts_dir=str(accounts_dir),
...
async def test_authenticator_avoids_fingerprint_signatures_already_in_use(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "tg_pool.api.telegram_auth.ClientFactory.build", lambda account: FakeClient(account)
    )
    authenticator = TelegramAuthenticator(
        fingerprint_file=str(source),
        accounts_dir=str(accounts_dir),
...
async def test_activation_manager_waits_for_launcher_two_factor(monkeypatch):
    ...
    monkeypatch.setattr("tg_pool.api.hero_sms_activation.request_number_v2", request)
    monkeypatch.setattr(
        "tg_pool.api.hero_sms_activation.get_activation_status",
        AsyncMock(return_value=("STATUS_OK", "54321")),
    )
    monkeypatch.setattr(
        "tg_pool.api.hero_sms_activation.close_activation", AsyncMock()
    )
