...
tg_pool/proxy/proxy_checker.py::check_proxy().
...
from tg_pool.proxy.proxy_checker import ProxyType, check_proxy
...
async def test_successful_check_carries_resolved_country():
    with (
        patch.dict("tg_pool.proxy.proxy_checker._CONNECTORS", {ProxyType.SOCKS5: _fake_connector()}),
        patch(
            "tg_pool.proxy.proxy_checker.country_for_proxy_host",
...
async def test_failed_check_still_carries_resolved_country():
    with (
        patch.dict(
            "tg_pool.proxy.proxy_checker._CONNECTORS",
...
async def test_invalid_config_does_not_attempt_country_lookup():
    ...
    with patch("tg_pool.proxy.proxy_checker.country_for_proxy_host", new=mock_country):
