"""tests/test_parsing_feature.py — tg_pool/features/parsing.py (MODE=parse)."""
...
from tg_pool.features import parsing
...
class TestBuildParsingStrategy:
    def test_defaults_to_group_members(self, monkeypatch):
        from tg_pool.extraction.data_extraction import GroupMembersStrategy
    ...
    def test_topic_reads_topic_id(self, monkeypatch):
        from tg_pool.extraction.data_extraction import TopicMessagesStrategy
...
class TestProxyPreflight:
    ...
    async def test_warns_but_keeps_dead_proxy_accounts_by_default(self, monkeypatch, caplog):
        ...
        with patch("tg_pool.proxy.proxy_checker.check_account_proxies", side_effect=fake_check):
            result = await parsing._proxy_preflight(accounts, logging.getLogger("test"))
...
class TestRun:
    ...
    async def test_uses_target_entity_when_parse_entities_unset(self, monkeypatch, tmp_path):
        ...
        with patch("tg_pool.orchestrator.orchestrate_extraction_only", new=AsyncMock(side_effect=fake_orchestrate)):
            await parsing.run(None, logging.getLogger("test"))
    ...
    async def test_exports_by_source(self, monkeypatch, tmp_path):
        ...
        with patch("tg_pool.orchestrator.orchestrate_extraction_only", new=AsyncMock(return_value=exporter)):
            await parsing.run(None, logging.getLogger("test"))
    ...
    async def test_redis_dedup_enabled_builds_and_closes_client(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch("tg_pool.orchestrator.orchestrate_extraction_only", new=AsyncMock(side_effect=fake_orchestrate)),
        ):
            await parsing.run(None, logging.getLogger("test"))
