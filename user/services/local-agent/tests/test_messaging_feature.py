"""tests/test_messaging_feature.py — tg_pool/features/messaging.py (MODE=send)."""
...
from tg_pool.features import messaging
...
class TestRun:
    ...
    async def test_full_run_calls_orchestrate_multi_source_with_target(self, monkeypatch):
        ...
        with patch(
            "tg_pool.orchestrator.orchestrate_multi_source", new=AsyncMock(side_effect=fake_orchestrate)
        ):
    ...
    async def test_account_folder_matching_nothing_logs_error_and_returns(self, monkeypatch, caplog):
        ...
        with patch("tg_pool.orchestrator.orchestrate_multi_source", new=AsyncMock()) as mock_orchestrate:
            await messaging.run(asyncio.Event(), logging.getLogger("test"))
    ...
    async def test_repeat_every_hours_passed_to_run_with_repeat(self, monkeypatch):
        ...
        with (
            patch("tg_pool.orchestrator.orchestrate_multi_source",
                  new=AsyncMock(return_value=BatchReport(total=1, succeeded=1))),
            patch("tg_pool.orchestrator.run_with_repeat", new=fake_run_with_repeat),
        ):
            await messaging.run(asyncio.Event(), logging.getLogger("test"))
