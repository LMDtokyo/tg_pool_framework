"""tests/test_licensing.py — LicenseService (tg_pool/licensing/service.py) and the
local license state cache (tg_pool/db/license_repository.py)."""
...
from tg_pool.db.license_repository import LicenseStateRepository, StoredLicenseState, ensure_license_state_table
from tg_pool.db.models import Base
from tg_pool.licensing.client import (
    LicenseActivation,
...
async def test_activation_with_a_valid_signature_succeeds(monkeypatch):
    ...
    monkeypatch.setattr("tg_pool.licensing.service.verify_activation", lambda **kwargs: True)
    service, _ = _service(
...
async def test_activation_with_an_invalid_signature_is_rejected(monkeypatch):
    ...
    monkeypatch.setattr("tg_pool.licensing.service.verify_activation", lambda **kwargs: False)
    service, _ = _service(
        [LicenseActivation(tier="month", activated_at=_NOW, expires_at=_NOW + timedelta(days=30), signature="sig")]
    )
    status = await service.activate("KEY", "hwid-1")
