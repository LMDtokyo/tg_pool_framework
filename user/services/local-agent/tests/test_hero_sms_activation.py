...
async def test_activation_manager_completes_received_sms(monkeypatch):
    ...
    monkeypatch.setattr("tg_pool.api.hero_sms_activation.get_activation_status", status)
    monkeypatch.setattr("tg_pool.api.hero_sms_activation.close_activation", close)
...
async def test_activation_manager_cancels_on_timeout(monkeypatch):
    monkeypatch.setattr(
        "tg_pool.api.hero_sms_activation.request_number_v2",
        AsyncMock(
            return_value={
                "activation_id": "123",
                "phone_number": "15550001111",
...
async def test_activation_manager_retries_one_higher_price_on_no_numbers():
    from tg_pool.api.hero_sms import HeroSmsApiError
