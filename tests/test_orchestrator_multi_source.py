...
from src.orchestrator import orchestrate_multi_source
...
async def test_auto_responder_attached_and_detached():
    ...
    with (
        patch("src.orchestrator.ClientPool", return_value=mock_pool),
        patch("src.orchestrator.extract_users", new=AsyncMock(return_value=users)),
        patch("src.orchestrator.send_notifications", new=AsyncMock(return_value=BatchReport(total=1, succeeded=1))),
    ):
        await orchestrate_multi_source(
            accounts=accounts,
            entity_identifiers=["@group"],
...
async def test_auto_responder_detached_even_on_extraction_error():
    ...
    with (
        patch("src.orchestrator.ClientPool", return_value=mock_pool),
        patch("src.orchestrator.extract_users", new=AsyncMock(side_effect=RuntimeError("boom"))),
    ):
        await orchestrate_multi_source(
            accounts=accounts,
            entity_identifiers=["@group"],
