"""tests/test_license_signature.py — src/licensing/signature.verify_activation.
...
into src/licensing/signature.py). Generated with
...
from license_server.signing import sign_activation
...
def _sign(**overrides):
    ...
    return fields, sign_activation(private_key, **fields)
...
def test_a_signature_from_the_wrong_key_fails_verification():
    ...
    signature = sign_activation(other_key, **fields)
    assert verify_activation(**fields, signature_hex=signature, public_key_hex=_TEST_PUBLIC_HEX) is False
