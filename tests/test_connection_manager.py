...
from src.accounts.connection_manager import ClientPool
...
async def test_connect_decrypts_session_before_building_client(tmp_path):
    ...
    with patch("src.accounts.connection_manager.ClientFactory.build", return_value=client) as build:
...
async def test_disconnect_reencrypts_session(tmp_path):
    ...
    client = make_mock_client()
    with patch("src.accounts.connection_manager.ClientFactory.build", return_value=client):
        pool = ClientPool([account], FAST_POLICY, session_encryption_key=key)
        await pool.initialize()
...
async def test_initialize_skips_account_that_times_out(monkeypatch):
    ...
    with patch("src.accounts.connection_manager.ClientFactory.build", side_effect=build_client):
