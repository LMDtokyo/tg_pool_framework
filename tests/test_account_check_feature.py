"""tests/test_account_check_feature.py — src/features/account_check.py (MODE=check_accounts)."""
...
from src.accounts.health_checker import AccountStatus, HealthResult, PoolHealthReport
from src.config import AccountConfig
from src.features import account_check
...
async def test_checks_primary_spare_and_tdata_accounts(monkeypatch):
    ...
    with patch("src.monitoring.monitor.LiveMonitor.print_pool_table") as mock_print:
        await account_check.run(None, logging.getLogger("test"))
...
async def test_deep_flag_read_from_env(monkeypatch):
    ...
    with patch("src.monitoring.monitor.LiveMonitor.print_pool_table"):
        await account_check.run(None, logging.getLogger("test"))
...
async def test_results_persisted_via_registry_close(monkeypatch):
    ...
    with (
        ...
        patch("src.monitoring.monitor.LiveMonitor.print_pool_table"),
    ):
        await account_check.run(None, logging.getLogger("test"))
