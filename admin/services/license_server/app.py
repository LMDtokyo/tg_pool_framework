...
A small, standalone FastAPI service, independent from the customer local
agent. It is the single source of
...
Deploy public_app:app and admin_app:app as separate processes. The combined
app in this module exists only for compatibility tests.
"""
...
async def lifespan(app: FastAPI):
    env_file = os.getenv("LICENSE_ENV_FILE", str(Path(__file__).resolve().parent / ".env"))
    load_dotenv(env_file)
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)-8s] %(name)s %(message)s")

    active_surface = getattr(app.state, "active_surface", "combined")
    if active_surface in {"admin", "combined"} and not os.getenv("LICENSE_ADMIN_API_KEY"):
        raise RuntimeError("LICENSE_ADMIN_API_KEY must be set -- refusing to start without admin auth")

    app.state.signing_key = (
        load_private_key_from_env() if active_surface in {"public", "combined"} else None
    )
